...
function hideBonus() {
    ...
    if (p.innerHTML != "1") {
        // TODO c.parentNode.removeChild(c);
    }
}
...
function createChartEE() {
    ...
    for (var i = 0; i < chart_elements.length; i++) {
		...
		var picco = chart_elements[i].getElementsByClassName("consumi_picco")[0];
		serPICCO[i + padresult] = picco.innerHTML == "" ? "" : picco.innerHTML;
        var totale = chart_elements[i].getElementsByClassName("consumi_totale")[0];
        lab[i + padresult] = getInitialMonthChar(mese.innerHTML) + (serPICCO[i + padresult] == "" ? " (-)" : " (" + serPICCO[i + padresult] + ")");
    ...
    var options = {
		...
		plugins: [
			Chartist.plugins.ctAxisTitle({
			  axisX: {
				axisTitle: 'Mese (Picco di potenza prelevata)',
...
function manageConsumoTempo(type)
{
    ...
    var chart_elements = document.getElementsByClassName('consumi_hiddenvalues');
 // TODO if(chart_elements.length < 12)
    	// TODO el.innerHTML = "Consumo da inizio fornitura";
// TODO else
    	// TODO el.innerHTML = "Consumo annuo";
...
function hideElements(type) 
{
    ...
    else if(type == "E")
    {
		...
		removeClassElementsIfClassesHaveSingleElementAndIsEmpty("messaggio-autolettura-tif", ".check-autolettura-tif");
        removeParentElementIfClassesHaveSomeValue("TR", ".valore-autolettura-tif", "data iniziale"); //rimuove sezione TIF se il tag della data iniziale non è presente
    }
    hideMessaggiBase();
	...
	removeClassElementsIfClassesHaveSingleElementAndIsEmpty("messaggio-split-payment", ".check-split-payment");
